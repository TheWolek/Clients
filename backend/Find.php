...
if(@$result->num_rows > 0) {
    ...
    while($row=$result->fetch_assoc()) {
        ...
        $colorStr = nl2br($row['numer_farby']);
        echo "<tr><td><input type='checkbox' name='select' value='{$row['id']}'></td><td id='idRow{$row['id']}'>{$row['id']}</td><td id='DaneRow{$row['id']}'>{$dane}</td><td id='dataRow{$row['id']}'>{$row['data_wizyty']}</td><td id='farbaRow{$row['id']}'>{$colorStr}</td>";
